...
def visualize_some_images(path_to_data):
    ...
    fig.tight_layout(pad=0.05) # to even out image display and unsquash text labels
    plt.show()
...
# DOWNLOAD DATA FROM ONLINE BUCKET TO LOCAL DIRECTORY
    ...
    for blob in blobs:
        ...
        if os.path.basename(joined_path) == '':
            ...
            if not os.path.isfile(joined_path):
                if not os.path.isdir(os.path.dirname(joined_path)): #VERIFY ALL FOLDERS THAT COME BEFORE DESIRED IMAGE, IF NOT EXIST THEN CREATE
                    os.makedirs(os.path.dirname(joined_path))
                blob.download_to_filename(joined_path)

# UPLOAD DATA FROM ONLINE BUCKET TO LOCAL DIRECTORY
